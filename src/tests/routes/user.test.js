...
describe('Users controller', () => {
    ...
    before(async () => {
        ...
        adminToken = jwt.encode({ sub: adminUser.id, role: adminUser.role }, process.env.JWT_SECRET);
        userToken = jwt.encode({ sub: testUser.id, role: testUser.role }, process.env.JWT_SECRET);
    });
    ...
    describe('Get single user by id', () => {
        it('Should return a status 403 if no token provided', async () => {
            const response = await chai.request(api).get(`/api/v1/users/${testUser.id}`);
            response.should.have.status(403);
        });
        ...
        it('Should return a status 404 if user does not exist', async () => {
            ...
            chai.expect(await User.findOne({ where: { id: nonExistingId } })).to.be.null;
            const response = await chai
                .request(api)
